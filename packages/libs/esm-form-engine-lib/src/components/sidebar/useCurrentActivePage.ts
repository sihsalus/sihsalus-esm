...
export const useCurrentActivePage = ({
  ...
  const initialLockTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    // Cleanup
    return (): void => {
      if (initialLockTimeoutRef.current) {
        clearTimeout(initialLockTimeoutRef.current);
        initialLockTimeoutRef.current = null;
      }
    };
  }, [isInitialized]);
  ...
  const requestPage = useCallback((pageId: string): void => {
