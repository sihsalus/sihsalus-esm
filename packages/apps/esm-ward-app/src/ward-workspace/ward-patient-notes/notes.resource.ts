...
import { type NoteObsData, type PatientNote, type RESTPatientNote, type UsePatientNotes } from './types';
  ...
  const patientNotes: Array<PatientNote> | null = useMemo(
    () =>
      data
        ? data
            .flatMap((encounter) => {
              return (encounter.obs as Array<NoteObsData>)?.reduce(
                (acc, obs) => {
                  if (conceptUuids.includes(obs.concept.uuid)) {
                    acc.push({
                      ...
                      encounterProvider: encounter.encounterProviders
