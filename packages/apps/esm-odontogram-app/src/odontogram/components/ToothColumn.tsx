...
const ToothColumn: React.FC<ToothColumnProps> = ({ toothId, children }) => {
  ...
  const handleRegisterFinding = useCallback(
    (params: { toothId: number; findingId: number; subOptionId?: number; color: { id: number; name: string }; designNumber?: number | null }) => {
  ...
  return (
    <div className="tim-tooth-col">
      ...
      {/* Finding count badge */}
      {findingsCount > 0 && (
        <span className="tim-badge">{findingsCount}</span>
      )}
