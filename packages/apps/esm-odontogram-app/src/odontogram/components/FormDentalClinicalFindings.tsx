...
const FormDentalClinicalFindings = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return opciones.filter(
      (op) => norm(op.nombre).includes(q) || String(op.id).includes(q)
    );
  }, [opciones, query]);
  ...
  return (
    <div className="odon-form">
      <div className="odon-form-body">
        ...
        <div className="odon-form-section odon-form-section--finding">
          ...
          <div className="odon-dropdown-wrap" ref={wrapperRef}>
            <button
              type="button"
              className={["odon-trigger", open ? "odon-trigger--open" : "", selectedItem ? "odon-trigger--filled" : ""].join(" ")}
              onClick={() => setOpen((v) => !v)}
            ...
            >
              {selectedItem ? (
                <>
                  ...
                  <span className="odon-trigger-clear" onClick={handleClear} role="button" aria-label="Limpiar">×</span>
                </>
              ) : (
            ...
            {open && (
              <div className="odon-popover" role="listbox">
                <div className="odon-popover-search-wrap">
                  ...
                  />
                  {query && (
                    <button className="odon-popover-clear-btn" type="button" onClick={() => setQuery("")}>×</button>
                  )}
                </div>
            ...
            <div className="odon-color-row">
              {selectedItem.colores.map((color: FindingColor) => (
                <button
                  key={color.id}
                  type="button"
                  className={["odon-color-swatch", selectedColor?.name === color.name ? "odon-color-swatch--active" : ""].join(" ")}
      ...
      {showInfo && selectedOption && SIGLAS_MAP[selectedOption] && (() => {
