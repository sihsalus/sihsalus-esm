 ...
 */
export function calculateSpacingDesignFromTeeth(
  space: SpaceData,
  findingId: number,
  teeth: ToothData[]
): number {
 ...
 */
export function recalculateSpacingDesigns(
  spaces: SpaceData[],
  findingId: number,
  teeth: ToothData[]
): SpaceData[] {
 ...
 */
export function calculateToothDesignFromSpaces(
  toothId: number,
  findingId: number,
  spaces: SpaceData[]
): number {
 ...
 */
export function recalculateToothDesigns(
  teeth: ToothData[],
  findingId: number,
  spaces: SpaceData[]
): ToothData[] {
  return teeth.map((tooth) => {
    ...
    const updatedFindings = tooth.findings.map((f, i) =>
      i === findingIdx ? { ...f, designNumber: newDesign } : f
    );
...
export function calculateLegendDesign(
  ...
  const leftActive =
    index > 0 &&
    legendSpaces[index - 1].findings.some((f) => f.findingId === findingId);
  const rightActive =
    index < legendSpaces.length - 1 &&
    legendSpaces[index + 1].findings.some((f) => f.findingId === findingId);
...
export function recalculateLegendDesigns(
  ...
  return legendSpaces.map((ls, i) => {
    ...
    return {
      ...ls,
      findings: ls.findings.map((f) =>
        f.findingId === findingId
          ? { ...f, designNumber: newDesign }
          : f
      ),
    };
  });
